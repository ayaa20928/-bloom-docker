...
app.post('/api/tasks', async (req, res) => {
  try {
    const { text, emoji, owner, tag, priority } = req.body;
    if (!text || !owner) return res.status(400).json({ success: false, error: 'text et owner requis' });
    const [result] = await pool.query(
      'INSERT INTO tasks (text, emoji, owner, tag, priority) VALUES (?, ?, ?, ?, ?)',
      [text, emoji || 'task', owner, tag || 'personnel', priority || 'normale']
    );
